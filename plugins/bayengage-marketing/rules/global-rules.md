...
Before planning around a segment, confirm it exists and check its size via `bayengage.segmentation`. A
...
### G10. Deterministic business rules belong to BayEngage, not to the agent.
...
Only data BayEngage can actually confirm for that specific contact. See
