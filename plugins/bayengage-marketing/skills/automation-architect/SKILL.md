...
metadata:
  ...
  targetbay.requires: bayengage.store_profile, bayengage.customer_intelligence, bayengage.product_intelligence, bayengage.order_intelligence, bayengage.segmentation, bayengage.automation, bayengage.automation_analytics, bayengage.suppression_and_consent
...
Declared in frontmatter; defined in [../../capabilities.yaml](../../capabilities.yaml). Concrete BayEngage
MCP tool mappings are **TODO** — see [../../docs/mcp-integration.md](../../docs/mcp-integration.md).

| Capability | Used for |
|---|---|
| `bayengage.store_profile` | Vertical, catalogue size, sending posture |
| `bayengage.customer_intelligence` | Lifecycle distribution, value bands, engagement, channel preference |
| `bayengage.product_intelligence` | Categories, attributes, price bands, affinity |
| `bayengage.order_intelligence` | Repeat interval, purchase frequency, AOV |
| `bayengage.segmentation` | Resolving and sizing candidate audiences |
| `bayengage.automation` | Reading existing topology; creating the designed journeys after approval |
| `bayengage.automation_analytics` | Per-node drop-off in existing journeys |
| `bayengage.suppression_and_consent` | Channel eligibility per audience |
...
- If `bayengage.messaging_sms` is unavailable, design email-only and record the gap.
...
| EXECUTE | Create journeys via `bayengage.automation` — one at a time | `mutation` |
...
- [ ] Every audience resolved and sized against `bayengage.segmentation` (A1)
...
| Situation | Response |
|---|---|
| `bayengage.automation` unavailable | **Blocked.** Report; do not design what cannot be built |
