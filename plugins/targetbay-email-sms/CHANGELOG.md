...
written that the inspected adjacent integration covers "event tracking, plus webhook events for
...
**The same correction, applied to the API.** The inspected integration's campaign resource exposes
read, list, send and reports — there is no create operation — and a large share of that repository's
published example workflows call one anyway. That is recorded in `docs/mcp-integration.md` so no skill
...
- `docs/mcp-integration.md` still quotes an adjacent repository's `bayengage_<verb>_<noun>` tool prefix.
