...
[![version](https://img.shields.io/badge/version-4.2.2-blue)](VERSION)
...
Installs the 38 skills and six slash commands.

</td></tr>
<tr><th>npm</th><td>

```bash
npx @targetbay/email-sms-skills --global
```
...
Download the `targetbay-email-sms@<version>` tarball from
...
```
/targetbay-email-sms:plan-month next month
/targetbay-email-sms:what-now
/targetbay-email-sms:win-back
```

Or just ask: *"Win back our lapsed customers."* · *"Why did this campaign underperform?"* ·
*"Prepare a Diwali campaign."*

With no TargetBay Email & SMS MCP connected, a skill will report itself **`blocked`** and name the capability it is
...
| Command | Does |
|---|---|
| `/targetbay-email-sms:plan-month` | A dated calendar for the period, campaign count derived not templated |
| `/targetbay-email-sms:what-now` | Open-ended scan; ranks what is most worth fixing or building |
| `/targetbay-email-sms:win-back` | Who among the lapsed is worth recovering, and where to stop |
| `/targetbay-email-sms:holiday <name>` | Whether a holiday is worth doing here, then the sequence |
| `/targetbay-email-sms:diagnose <campaign>` | Why a campaign underperformed, and a testable fix |
| `/targetbay-email-sms:audit-automations` | Portfolio audit; gaps, splits, retirements, sequenced |

---

## What this is

TargetBay Email & SMS already exposes its platform capabilities through an MCP server. That tells an agent *what it
...
```
TargetBay Email & SMS MCP    =  what the agent CAN do
TargetBay Email & SMS Skills =  how the agent SHOULD accomplish a marketing objective
```
...
monthly-marketing-planner
        │
        ├── reads store context via TargetBay Email & SMS MCP
...
| | Skills |
|---|---|
| **Revenue** | [revenue-growth](skills/revenue-growth/SKILL.md) · [revenue-analysis](skills/revenue-analysis/SKILL.md) · [opportunity-discovery](skills/opportunity-discovery/SKILL.md) · [aov-growth](skills/aov-growth/SKILL.md) · [cross-sell](skills/cross-sell/SKILL.md) · [upsell](skills/upsell/SKILL.md) · [stock-and-price-alerts](skills/stock-and-price-alerts/SKILL.md) · [offer-strategy](skills/offer-strategy/SKILL.md) · [product-recommendation-strategy](skills/product-recommendation-strategy/SKILL.md) |
...
| **Planning** | [marketing-calendar](skills/marketing-calendar/SKILL.md) · [monthly-marketing-planner](skills/monthly-marketing-planner/SKILL.md) · [store-onboarding](skills/store-onboarding/SKILL.md) · [campaign-conflict-resolver](skills/campaign-conflict-resolver/SKILL.md) |
...
| **Email quality** | [email-quality-auditor](skills/email-quality-auditor/SKILL.md) · [deliverability-qa](skills/deliverability-qa/SKILL.md) · [email-render-qa](skills/email-render-qa/SKILL.md) · [dynamic-content-personalizer](skills/dynamic-content-personalizer/SKILL.md) |
...
```
skills/        38 skills, one SKILL.md each — objective → decisions → plan
rules/         9 numbered, citable rule sets that bind every skill
knowledge/     10 marketing principles documents, vendor-agnostic
...
- **Never personalises on data TargetBay Email & SMS cannot verify for that contact**

`blocked` and `partial` are first-class results. See [rules/](rules/README.md).

## Status

**4.2.2 — foundation, published.** Contracts established, reasoning real, workflows not yet hardened
...
`email_sms.messaging_sms` is declared but **unverified** — no inspected TargetBay Email & SMS implementation
exposes SMS dispatch. Skills degrade to email-only when it is absent.
...
metadata:
  ...
  targetbay.requires: email_sms.customer_intelligence, email_sms.order_intelligence, …
...
The agent also needs access to TargetBay Email & SMS MCP. Without it, skills will correctly report themselves as
