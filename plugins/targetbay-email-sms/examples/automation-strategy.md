...
`email_sms.automation` for the existing portfolio and topology; `email_sms.automation_analytics` for
per-node performance; `email_sms.customer_intelligence` for lifecycle distribution;
`email_sms.order_intelligence` for repeat intervals and AOV; `email_sms.product_intelligence` for
categories and attributes; `email_sms.segmentation` for sizing.
