...
`email_sms.order_intelligence` for purchase history and the store's own intervals;
`email_sms.customer_intelligence` for lapse state, prior value and engagement recency;
`email_sms.suppression_and_consent` for reachability and complaint history;
`email_sms.campaign_analytics` for prior win-back attempts; `email_sms.product_intelligence` for what has
...
checked first; `email_sms.messaging_sms` is unverified, so the plan degrades to email-only if it is
absent.
