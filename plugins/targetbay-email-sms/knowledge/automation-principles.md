...
These are abstract planning types. The concrete node types a TargetBay Email & SMS automation actually
supports are a platform question — see [../docs/mcp-integration.md](../docs/mcp-integration.md).
...
Per-node drop-off from `email_sms.automation_analytics` tells you which node is the problem. Adding a
