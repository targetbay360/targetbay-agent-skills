...
what this store actually is, what to set up first across all four products, and in what order.
...
measured, which are borrowed from its vertical, and which do not exist at all — and then sequencing four
...
- **Not** a replacement for the four product plugins — it sequences them, it does not duplicate them
...
| [Cross-Product Onboarding Blueprint](skills/onboarding-blueprint/SKILL.md) | What to set up first, across all four products | `plan` |
...
## Three things this package insists on
...
Onboarding is the only moment when all four programmes are designed at once, so it is where ownership of
...
this release. See [docs/mcp-integration.md](docs/mcp-integration.md), which also records the nine
