...
Onboarding is where it gets fixed, because onboarding is the only moment when all three programmes are
designed at once and nothing is live yet. The onsite work onboarding also carries is outside the budget
entirely — see [#X3](#x3-onsite-work-never-consumes-contact-budget).

---

### X1. One contact budget per customer per period, owned by the platform, not by a product.
The budget is a single number covering email, SMS, review requests and programme messages. Each product
honouring its own cap is three products jointly exceeding the real one. Where the platform can
...
| Onsite | Onboarding | No product owns it; it acts on a visitor already present, see [#X3](#x3-onsite-work-never-consumes-contact-budget) |
...
### X3. Onsite work never consumes contact budget.
...
The number is computed across all three products, shown in the approval request, and approved on its face
...
If no unified frequency and consent view exists across the three products, the budget is stated as
