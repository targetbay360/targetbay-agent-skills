...
content and system integration — each with its trigger, its preconditions, the MCP capabilities it
consumes, the guardrails it must carry and what to measure afterwards.
...
```
targetbay-marketing-automation-recipes/
├── SKILL.md                                    # Start here — the MCP contract and routing
└── references/
    ├── how-to-read-a-recipe.md                 # The eight fields; the capability map; the gaps
    ├── guardrails.md                           # Dedupe, suppression, frequency, approval, events
    ...
    ├── integration-recipes.md                  # Contact sync, lead capture, event stream
    └── not-shipped.md                          # Patterns that were refused, and why
```

## Quick start

Open `SKILL.md`. It states the MCP contract first — including the capabilities that do not
exist — then routes to the right reference, and lists what this skill will not do and why.

If you are wiring the first automation for a store, confirm the connected MCP exposes the
capabilities you need, then build the event-stream recipe in `references/integration-recipes.md`. Every event-triggered recipe assumes that
layer works.
