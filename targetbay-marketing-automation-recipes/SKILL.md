...
What follows was established by reading an inspected integration against the platform API, not from
...
A naming note. The product is **TargetBay Email & SMS**. The inspected integration and the API path
...
recipe then selects and sends. A large share of the published example workflows for this platform
call a campaign-create operation that does not exist; that was found by reading the integration
rather than by running them. Each recipe file states the working shape in its opening lines.
