...
create a campaign, send it — cannot be built against this surface. A large share of the published
example workflows for this platform do exactly that, calling a campaign-create operation the
integration never implements. Those nodes return nothing rather than failing loudly, which is the
worst way for it to break: the workflow reports success and no email is sent.
...
before assuming either way — the integration inspected here is one implementation at
