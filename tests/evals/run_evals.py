...
def load_skills(plugin: Path) -> dict[str, dict]:
...
def build_vectors(skills: dict[str, dict]) -> dict[str, float]:
    """Inverse document frequency over this plugin's skills, so vocabulary the whole
    product shares ("customer", "review", "member") does not decide the match."""
    ...
    for s in skills.values():
        ...
        for term, count in s["description_terms"].items():
            weighted[term] += 3.0 * count * idf.get(term, 0.1)
        for term, count in s["when_terms"].items():
            weighted[term] += 1.0 * count * idf.get(term, 0.1)
        norm = math.sqrt(sum(v * v for v in weighted.values())) or 1.0
        s["vector"] = {t: v / norm for t, v in weighted.items()}
    return idf


def ranker(skills: dict[str, dict], idf: dict[str, float]):
...
if not plugin_dirs:
    sys.exit("no plugins found; run from inside the repository")

all_cases: list[dict] = []
all_skills: dict[str, dict] = {}          # "plugin/skill" -> record, for the lock
seen_ids: dict[str, str] = {}
rank1_hits = 0
rank1_eligible = 0
per_plugin: list[tuple[str, int, int]] = []

for plugin in plugin_dirs:
    pname = plugin.name
    skills = load_skills(plugin)
    if not check("schema", bool(skills), f"{pname}: no skills found"):
        continue
    idf = build_vectors(skills)
    rank = ranker(skills, idf)

    cases: list[dict] = []
    suite_dir = EVALS / "golden-prompts" / pname
    for path in sorted(suite_dir.glob("*.yaml")) if suite_dir.is_dir() else []:
        doc = yaml.safe_load(path.read_text()) or {}
        check("schema", "suite" in doc and "cases" in doc,
              f"{pname}/{path.name}: missing 'suite' or 'cases'")
        for case in doc.get("cases", []) or []:
            case["_suite"] = doc.get("suite", path.stem)
            case["_file"] = f"{pname}/{path.name}"
            case["_plugin"] = pname
            cases.append(case)

    check("schema", bool(cases), f"{pname}: no eval cases found in golden-prompts/{pname}/")
    all_cases += cases
    per_plugin.append((pname, len(cases), len(skills)))
    for name, s in skills.items():
        all_skills[f"{pname}/{name}"] = s
    ...
    for case in cases:
        ...
        unknown = set(case) - CASE_KEYS - {"_suite", "_file", "_plugin"}
        check("schema", not unknown, f"{cid}: unknown keys {sorted(unknown)}")
        check("schema", cid not in seen_ids,
              f"duplicate case id '{cid}' in {case['_file']} and {seen_ids.get(cid)}")
        seen_ids[cid] = case["_file"]
        check("schema", bool(str(case.get("prompt", "")).strip()), f"{cid}: empty prompt")
        check("schema", bool(str(case.get("notes", "")).strip()),
              f"{cid}: empty notes — say why this case exists")

    # --------------------------------------------------------------- skill-refs
    for case in cases:
        cid = case["id"]
        expect = case.get("expect_skill")
        check("skill-refs", expect in skills,
              f"{cid}: expect_skill '{expect}' is not a skill in {pname}")
        for alt in case.get("accept_skills", []) or []:
            check("skill-refs", alt in skills,
                  f"{cid}: accept_skills names unknown skill '{alt}' in {pname}")
        if expect in skills:
            declared = set(meta_list(skills[expect]["fm"], "composes"))
            for dep in case.get("expect_composes", []) or []:
                check(
                    "skill-refs", dep in declared,
                    f"{cid}: expects {expect} to compose '{dep}', "
                    f"but its frontmatter does not declare it",
                )
    ...
    for case in cases:
        ...
        for ref in case.get("must_cite", []) or []:
            ...
            path = plugin / "rules" / fname
            if not check("rule-refs", path.is_file(),
                         f"{cid}: rule file '{fname}' does not exist in {pname}/rules/"):
                continue
            text = rule_cache.setdefault(fname, path.read_text())
            check("rule-refs", re.search(rf"^### {anchor}\.", text, re.MULTILINE) is not None,
                  f"{cid}: rule '{anchor}' not found in {pname}/rules/{fname}")
    ...
    for case in cases:
        ...
        ok = check(
            ...
            f"{cid}: no accepted skill within top {bar} of {pname}; best was "
            f"{f'#{position}' if position else 'unranked'}, top 3 were {names[:3]}",
        )
        if ok and position == 1 and len(ranked) > 1 and ranked[0][1] > 0:
            margin = (ranked[0][1] - ranked[1][1]) / ranked[0][1]
            if margin < 0.10:
                warnings.append(
                    f"{cid}: thin lexical margin over '{names[1]}' ({margin:.0%}) "
                    f"— descriptions may overlap"
                )
    ...
    check("coverage", not missing_cover,
          f"{pname}: skills with no golden prompt: {', '.join(missing_cover)}")

    # store the plugin's ranker and catalogue for --emit
    for case in cases:
        case["_catalogue"] = "\n".join(
            f"- **{n}** — {s['fm']['description']}" for n, s in sorted(skills.items())
        )

check("schema", bool(all_cases), "no eval cases found in any plugin")

print(f"\ngolden prompts — {len(all_cases)} cases across {len(per_plugin)} plugins\n")
...
def skill_version(case: dict) -> str | None:
    record = all_skills.get(f"{case['_plugin']}/{case.get('expect_skill')}")
    if not record:
        return None
    return (record["fm"].get("metadata") or {}).get("targetbay.version")


current = {
    c["id"]: {"hash": canonical(c), "skill_version": skill_version(c)}
    for c in all_cases
    if f"{c['_plugin']}/{c.get('expect_skill')}" in all_skills
}
expect_by_id = {c["id"]: c["expect_skill"] for c in all_cases}
...
if "--update" in sys.argv:
    ...
    for cid, now in sorted(current.items()):
        ...
        check(
            ...
            f"{cid}: expectations changed while '{expect_by_id[cid]}' stayed at version "
...
if "--emit" in sys.argv:
    ...
    for case in all_cases:
        ...
        body = [
            f"# Eval {case['id']} — plugin `{case['_plugin']}`, suite `{case['_suite']}`",
            "",
    ...
    print(f"prompt packs written to {OUT.relative_to(ROOT)}/ — {len(all_cases)} files\n")
...
for pname, ncases, nskills in per_plugin:
    print(f"  {pname}: {ncases} cases, {nskills} skills")
